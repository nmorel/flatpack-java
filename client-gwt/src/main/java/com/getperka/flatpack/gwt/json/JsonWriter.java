...
/**
 * Writes a JSON (<a href="http://www.ietf.org/rfc/rfc4627.txt">RFC 4627</a>) encoded value to a stream, one token at a
 * time. The stream includes both literal values (strings, numbers, booleans and nulls) as well as the begin and end
 * delimiters of objects and arrays. <h3>Encoding JSON</h3> To encode your data as JSON, create a new {@code JsonWriter}
 * . Each JSON document must contain one top-level array or object. Call methods on the writer as you walk the
 * structure's contents, nesting arrays and objects as necessary:
 * <ul>
 * <li>To write <strong>arrays</strong>, first call {@link #beginArray()}. Write each of the array's elements with the
 * appropriate {@link #value} methods or by nesting other arrays and objects. Finally close the array using
 * {@link #endArray()}.
 * <li>To write <strong>objects</strong>, first call {@link #beginObject()}. Write each of the object's properties by
 * alternating calls to {@link #name} with the property's value. Write property values with the appropriate
...
public class JsonWriter
{
     ...
     * must be escaped: quotation mark, reverse solidus, and the control characters (U+0000 through U+001F)." We also
     * escape '\u2028' and '\u2029', which JavaScript interprets as newline characters. This prevents eval() from
     * failing with a syntax error. http://code.google.com/p/google-gson/issues/detail?id=341
     */
     ...
     * by <a href="http://www.ietf.org/rfc/rfc4627.txt">RFC 4627</a>. Setting the writer to lenient permits the
     * following:
     * <ul>
